...
# --- Assurer l'import des packages locaux (avant site-packages) ---
ROOT = os.path.dirname(os.path.abspath(os.path.join(__file__, "..")))
if ROOT not in sys.path:
    sys.path.insert(0, ROOT)

# --- ORM / Session ---
from orm.db import SessionLocal  # ta factory de sessions
...
# --- ORM models (pour check d'existence) ---
...
def get_or_create_user(repo: UserRepo, *, email: str, username: str, role: UserRole, password: str) -> User:
    ...
    if existing:
        u = User(
            ...
            date_joined=datetime.fromisoformat(existing.date_joined) if isinstance(existing.date_joined, str) else existing.date_joined,
            created_at=datetime.fromisoformat(existing.created_at) if isinstance(existing.created_at, str) else existing.created_at,
            updated_at=datetime.fromisoformat(existing.updated_at) if isinstance(existing.updated_at, str) else existing.updated_at,
            last_login=None,
        )
        return u
...
def get_or_create_client(repo: ClientRepo, *, email: str, full_name: str, phone: str, company_name: str, sales_contact_id: int | None) -> Client:
...
def get_or_create_contract(
    ...
    *,
    ...
    # clé naturelle simple pour éviter doublons de seed
    existing = (
        repo.s.query(ContractModel)
        .filter(
            ContractModel.client_id == client_id,
            ContractModel.total_amount == str(total_amount),
    ...
    if existing:
        return Contract(
            ...
            amount_due=Decimal(str(existing.amount_due)),
    ...
    *,
    ...
    if existing:
        return Event(
            ...
            event_start=datetime.fromisoformat(existing.event_start) if isinstance(existing.event_start, str) else existing.event_start,
            event_end=datetime.fromisoformat(existing.event_end) if isinstance(existing.event_end, str) else existing.event_end,
...
def run_seed(password_plain: str = "Azerty123$") -> None:
    with session_scope() as s:
        ...
        gestion = get_or_create_user(
            user_repo, email="gestion@epic-events.com", username="gestion", role=UserRole.GESTION, password=password_plain
        )
        commercial = get_or_create_user(
            user_repo, email="commercial@epic-events.com", username="commercial", role=UserRole.COMMERCIAL, password=password_plain
        )
        support = get_or_create_user(
            user_repo, email="support@epic-events.com", username="support", role=UserRole.SUPPORT, password=password_plain
        )
